...
/**
 * Initialise the Chatwoot widget and set the verified user identity.
 *
 * identifierHash: HMAC-SHA256(secret, email) generated by your n8n verify_code
 * webhook using the Chatwoot "Identity Verification Token" from inbox settings.
 * Without it Chatwoot marks the contact as unverified and may create duplicates.
 */
const launchChatwoot = (name: string, email: string, identifierHash?: string) => {
  ...
  // Wait for the SDK script tag to finish loading (set in Layout.astro)
  if (window.__chatwootReady) {
    window.__chatwootReady.then(startSDK);
  } else {
    startSDK();
  }
};
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (payload?.success) {
        ...
        launchChatwoot(u.name, u.email, identifierHash);
      } else {
